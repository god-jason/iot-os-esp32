...
/*
获取CPU使用率，需要定时取占用时间差值来计算CPU占用率
@api    rtos.cpu_usage()
@return int CPU 总时间
@return int IDLE 时间
@usage
-- 打印CPU占用
log.info("cpu.lua", rtos.cpu_usage())
*/
static int l_rtos_cpu_usage(lua_State *L) {    
    uint32_t total, idle;
    int ret = luat_task_cpu_usage(&total, &idle);
    lua_pushinteger(L, total);
...
static const rotable_Reg_t reg_rtos[] =
{
    ...
    { "cpu_usage",           ROREG_FUNC(l_rtos_cpu_usage)},
